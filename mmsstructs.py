...
def find_crossings(array, timeseries, gap_time):
    ...
    arr_compare = abs(arr_sign[:-1]-arr_sign[1:])
        # compares each element to next one
    arr_crossing = (arr_compare == 2)
        # true if there is a crossing
...
def refine_crossings(array, indices, directions, width, min_height):
    ...
    '''
    cleaned_indices_arr = np.array(indices)
        # to np array for processing purposes
    max_index = len(array)
    ...
    for n, direc in enumerate(directions):
        if direc > 0:  # crossing from neg to pos
            if 0 < n < (len(indices)-1):  # index on each side
        ...
        if (abs(prev_ext) < min_height):
            ...
            while i < len(directions):
                if (abs(next_exts[i]) >= min_height and
                    directions[i] == direc):
                    # trend is continuing up or down across 0
                    mask = np.append(mask, [True])
                    break
                elif (abs(next_exts[i]) >= min_height):
                    # up then down or vice versa
                    mask = np.append(mask, [False])
                    break
                i += 1
...
def structure_extent(array, indices, times, directions, width, min_height,
    ...
    Inputs:
        ...
        times- numpy array of the timeseries of the array which has zero
               crossings
    ...
    min_indices = tuple(filter(lambda x: -1*array[x] > min_height,
                               min_indices_tmp[0]))
    reduced_width = width
    while (len(max_indices) == 0) or (isinstance(max_indices[0], np.ndarray)):
        # need at least one maximum index
        if reduced_width == 0:
    ...
    reduced_width = width
    while (len(min_indices) == 0) or (isinstance(min_indices[0], np.ndarray)):
        # need at least one minimum index
        if reduced_width == 0:
        ...
        reduced_width -= 1  # filter out bad zero crosses using extrema height
     
    size_list = []
    times_list = []
    mins_arr = np.asarray(min_indices)  # tuple to numpy array for searching
    ...
    for n, current_index in enumerate(indices):
        if directions[n] == 1:  # crossing from negative to positive
            left_idx = np.searchsorted(mins_arr, current_index,
                                       side='left') - 1
            ...
            left_idx = np.searchsorted(maxes_arr, current_index,
                                       side='left') - 1
...
def structure_classification(crossing_direction, vex_direction, vex_quality,
    ...
    if ((jy_direction > 0) and (vex_direction > 0) and
