...
def structure_hist_maker(data,structure_type,out,log=False):
    ...
    histogram_plotter(ax1,data['1'],labels_M[0],all_limits,n_bins=nbins,
                      logscale=log)
    histogram_plotter(ax2,data['2'],labels_M[1],all_limits,n_bins=nbins,
                      logscale=log)
    histogram_plotter(ax3,data['3'],labels_M[2],all_limits,n_bins=nbins,
                      logscale=log)
    histogram_plotter(ax4,data['4'],labels_M[3],all_limits,n_bins=nbins,
                      logscale=log)
    histogram_plotter(ax5,total_data,labels_tot,all_limits,n_bins=nbins,
                      logscale=log)
...
def datetime_converter(time_dt):
    '''
    converts datetime objects to MMS CDF times
    '''
    start_date=dt.datetime(2000,1,1,hour=11,minute=58,second=55,
                                 microsecond=816000)
...
def section_maker(indices,maxes,mins,max_index,min_index=0):
    ...
    This is basically me putting my horrible spaghetti code into a function
    This way it's "well-organized" code
...
for M in MMS:
    ...
    TT_time_j=datetime_converter(time_reg_jcurl) #time to nanosecs for interpolating
    time_reg_b=np.array(time_converter(TT_time_b)) #time as datetime obj np arr
    time_reg_ni=np.array(time_converter(TT_time_ni))
    time_reg_ne=np.array(time_converter(TT_time_ne))
    bz=b_field[:,2]
    jy=j_curl[:,1]
    vex_fpi=ve_fpi[:,0]
    vix=vi[:,0]
    ...
    for i in range(len(crossing_indices_bz)):
        ...
        if (REPLOT):
            ...
            ax6.axis('off')
            tseries_plotter(fig,ax1,time_b_cut,bz_cut,b_labels,plot_limits) #plot B
            tseries_plotter(fig,ax2,time_j_cut,jy_cut,j_labels,plot_limits) #plot jy
            tseries_plotter(fig,ax3,time_ni_cut,ni_cut,n_labels,plot_limits,
                          legend=n_legend[0]) #plot ni
            tseries_plotter(fig,ax3,time_ne_cut,ne_cut,n_labels,plot_limits,
                          legend=n_legend[1]) #plot ne (same axis)
            tseries_plotter(fig,ax3,time_ne_cut,ne_smooth_cut,n_labels,
...
#                            plot_limits,legend=n_legend[4]) #plot ne error for comparison
            tseries_plotter(fig,ax4,time_j_cut,vex_cut,v_labels,plot_limits,
                          legend=v_legend[0])#plt vex from curlometer
            tseries_plotter(fig,ax4,time_ne_cut,vex_fpi_cut,v_labels,
                            plot_limits,legend=v_legend[1])#plot vex from moments
            tseries_plotter(fig,ax4,time_ni_cut,vix_cut,v_labels,plot_limits,
                          legend=v_legend[2])#plot vi from moments    
            tseries_plotter(fig,ax5,time_ni_cut,vix_fluct,fluct_labels,
                            plot_limits,legend=fluct_legend[0])#plot vi fluctuations  
            tseries_plotter(fig,ax5,time_j_struct,vex_fluct,fluct_labels,
                            plot_limits,legend=fluct_legend[1])#plot ve fluctuations 
            #add horizontal and vertical lines to plot (crossing + extent)
            line_maker([ax1,ax2,ax3,ax4,ax5],crossing_times[i],
            ...
            if (i % 30 == 0):
                ...
                ax2=plt.subplot2grid(gridsize,(1,0))   
                tseries_plotter(fig,ax1,time_b_cut,bz_cut,b_labels,plot_limits) #plot B
                tseries_plotter(fig,ax2,time_b_large,bz_large,
